...
class Packet:
    ...
    def to_bytes(self) -> bytes:
        ...
        return struct.pack("<I", total_len) + struct.pack("<H", self.msg_type) + self.payload
    ...
    def from_bytes(cls, data: bytes) -> tuple["Packet", bytes]:
        ...
        if len(data) < 4 + length:
            raise ValueError(f"Incomplete packet: expected {length} bytes, got {len(data) - 4}")
...
class UserLoginResponse:
    ...
    def to_packet(self) -> Packet:
        ...
        return Packet(
            msg_type=MessageType.USER_LOGIN_RESPONSE,
