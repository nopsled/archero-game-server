  ...
  static bypass(isDebugging = false) {
    Java.perform(() => {
      ...
      try {
        ...
        TrustManagerImpl_Activity_1.checkTrustedRecursive.implementation = (certs, ocspData, tlsSctData, host, clientAuth, untrustedChain, trustAnchorChain, used) => {
          if (isDebugging)
      ...
      try {
        ...
        TrustManagerImpl_Activity_2.verifyChain.implementation = (untrustedChain, trustAnchorChain, host, clientAuth, ocspData, tlsSctData) => {
          if (isDebugging)
...
var ntohsPtr = Module.findExportByName(null, "ntohs");
var inet_addrPtr = Module.findExportByName(null, "inet_addr");
if (getaddrinfoPtr != null && connectPtr != null && sendPtr != null && recvPtr != null && ntohsPtr != null && inet_addrPtr != null) {
  getaddrinfoFunction = new NativeFunction(getaddrinfoPtr, "int", [
    "pointer",
    "pointer",
    "pointer",
    "pointer"
  ]);
  connectFunction = new NativeFunction(connectPtr, "int", ["int", "pointer", "int"]);
  sendFunction = new NativeFunction(sendPtr, "int", ["int", "pointer", "int", "int"]);
  recvFunction = new NativeFunction(recvPtr, "int", ["int", "pointer", "int", "int"]);
  ntohs = new NativeFunction(ntohsPtr, "uint16", ["uint16"]);
  inet_addr = new NativeFunction(inet_addrPtr, "int", ["pointer"]);
  connect = new NativeFunction(connectPtr, "int", ["int", "pointer", "uint"]);
}
var getaddrinfoFunction;
var connectFunction;
var sendFunction;
var recvFunction;
var ntohs;
...
var Patcher = class {
  ...
  static PatchConnect(newIP, ports = [443], isDebugging = false) {
    if (connectPtr != null) {
      Interceptor.replace(connectPtr, new NativeCallback((socket, address, addressLen) => {
        var endpoint = {
          ip: "",
          port: 0
        };
