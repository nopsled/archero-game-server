...
function log(category: string, op: string, target: string, detail?: string): void {
  const icons: Record<string, string> = {
    file: "📂", prefs: "🔑", asset: "📦", json: "📋", binary: "💾", 
    stream: "📖", save: "💿", crypto: "🔐", packet: "📡", net: "🌐", dns: "🔍"
  };
  const icon = icons[category] || "📁";
  
  console.log(`${ts()} [${category.toUpperCase().padEnd(7)}] ${icon} ${op}: ${preview(target, 50)}`);
...
function printFields(fields: Record<string, any>, indent: string = "│   "): void {
  for (const [key, value] of Object.entries(fields)) {
    if (value && typeof value === "object" && value.fields) {
      ...
      const extra = value.len !== undefined ? `, len=${value.len}` : 
...
function hookSystemIO(): void {
  ...
  try {
    ...
    try {
      ...
      const fileMethods = ["OpenRead", "ReadAllText", "WriteAllBytes", "Create", "Delete", "Exists"];
    ...
    for (const m of encryptMethods) {
      try {
        netEncrypt.method(m).implementation = function(...args: any[]) {
          ...
          encryptionEvents.push({ t: elapsed(), method: m, input: input.substring(0, 100), output: output.substring(0, 100) });
...
function printSummary(): void {
  ...
  // Categories
  const categories = ["file", "prefs", "json", "save", "crypto", "packet", "net", "dns", "asset", "stream"];
  console.log(`\n📈 Events by Category:`);
  ...
  connections.slice(0, 10).forEach(c => console.log(`   - ${c.ip}:${c.port} @ ${c.t.toFixed(2)}s`));
  
  console.log(`\n🔐 Encryption Events (${encryptionEvents.length}):`);
  encryptionEvents.slice(0, 10).forEach(e => console.log(`   - [${e.t.toFixed(2)}s] ${e.method}: ${e.input.substring(0, 30)}...`));
  ...
  const saveGroups = saveDataEvents.reduce((acc, e) => {
    acc[e] = (acc[e] || 0) + 1;
    return acc;
  ...
  const summary = {
    ...
    categories: Object.fromEntries(categories.map(c => [c, events.filter(e => e.category === c).length])),
    dns: Array.from(dnsLookups.keys()),
    connections: connections.slice(0, 10),
    packets: packetCaptures.map(p => ({ t: p.t, dir: p.dir, name: p.name })),
    encryption: encryptionEvents.slice(0, 20).map(e => ({ t: e.t, method: e.method, input: e.input.substring(0, 50) })),
