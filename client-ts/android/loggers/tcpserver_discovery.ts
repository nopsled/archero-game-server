/**
 * TcpServer Discovery Tool
 *
 * Hooks all Il2Cpp classes in the "TcpServer" namespace and logs every method call
 * during the first N seconds after the Unity runtime becomes available.
 *
 * Usage:
 *   cd client
 *   bun run build:tcpserver-discovery
 *   frida -D <device> -N com.habby.archero -l android/build/tcpserver_discovery.js
 */

import "frida-il2cpp-bridge";

console.log("[TcpServerDiscovery] Script loaded");

const TARGET_NAMESPACE = "TcpServer";
...
function hookClass(fullName: string, clazz: Il2Cpp.Class): number {
  ...
  if (hookedCount > 0) {
    ...
    if (hookedClasses.size % 25 === 0) {
      console.log(
        `[TcpServerDiscovery] Progress: ${hookedClasses.size} classes (${totalHookedMethods} methods)`
      );
    }
  }
...
console.log(
  `[TcpServerDiscovery] Will hook namespace "${TARGET_NAMESPACE}" for ${DISCOVERY_DURATION_MS / 1000}s...`
);

Il2Cpp.perform(() => {
  discoveryStartTime = Date.now();
  console.log("[TcpServerDiscovery] Il2Cpp runtime ready, scanning classes...");

  // Prefer Assembly-CSharp, but fall back to scanning all loaded images if needed.
  ...
  if (foundClasses === 0) {
    console.log(`[TcpServerDiscovery] No classes found in namespace "${TARGET_NAMESPACE}"`);
  } else {
    console.log(
      `[TcpServerDiscovery] Hooked ${hookedClasses.size} classes (${totalHookedMethods} methods)`
    );
  }

  setTimeout(() => {
    console.log("\n[TcpServerDiscovery] Discovery period complete!");
    console.log("TCPSERVER_DISCOVERY_LOG_START");
    console.log(JSON.stringify(generateSummary(), null, 2));
    console.log("TCPSERVER_DISCOVERY_LOG_END");
