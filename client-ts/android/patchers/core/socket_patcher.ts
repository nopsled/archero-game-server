...
const ipsByHostname = new Map<string, Set<string>>();
const recentWatchedHostByTid = new Map<number, { ts: number; host: string }>();

type TransportProto = "tcp" | "udp" | "unknown";
...
type ObservedEndpoint = {
  ...
  appProtocols: Set<string>;
};

const observedEndpoints = new Map<string, ObservedEndpoint>();
...
let connectRedirect: ConnectRedirectConfig = {
  enabled: false,
  targetIp: "127.0.0.1",
  ports: [],
  allowlistHosts: [],
...
function addHostnameIp(host: string, ip: string) {
...
export class Patcher {
  ...
  public static ConfigureConnectRedirect(config: Partial<ConnectRedirectConfig> = {}) {
    connectRedirect = { ...connectRedirect, ...config, enabled: config.enabled ?? true };
    console.log(
      `[${Utilities.now()}] connectRedirect enabled=${connectRedirect.enabled} targetIp=${connectRedirect.targetIp} ports=${JSON.stringify(
        connectRedirect.ports
      )} allowlistHosts=${JSON.stringify(connectRedirect.allowlistHosts)} allowlistIps=${JSON.stringify(
        connectRedirect.allowlistIps
      )}`
    );
  }
        ...
        onEnter(args) {
          try {
            ...
            if (family === 2 /* AF_INET */) {
              ...
              if (
                connectRedirect.enabled &&
                connectRedirect.ports.includes(port) &&
                (allowAll || hostAllowed || ipAllowed)
              ) {
                const reason = allowAll ? "all" : hostAllowed ? "host" : "ip";
                ipPtr.writeInt(
                  inet_addr(Memory.allocUtf8String(connectRedirect.targetIp)) as number
                );
                trackedSockets.add(fd);
                updateSocketMeta(fd, {
                  family,
                ...
                if (connectDebug) {
                  console.log(
                    `[${Utilities.now()}] connect(fd=${fd}) ${originalIp}:${port} -> ${connectRedirect.targetIp}:${port} (redirect:${reason})`
                  );
                }
              }
