📦
156667 /android/loggers/auth_flow_discovery.js
✄
...
// android/loggers/auth_flow_discovery.ts
console.log("\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557");
...
function hookNativeNetwork() {
  ...
  if (getaddrinfoPtr) {
    ...
    Interceptor.replace(getaddrinfoPtr, new NativeCallback((name, service, hints, res) => {
      ...
      const result = getaddrinfoFn(name, service, hints, res);
      if (result === 0 && hostname !== "<null>") {
        try {
          const list = res.readPointer();
          if (!list.isNull()) {
            let cur = list;
            let safety = 0;
  ...
  try {
    ...
    for (const fullName of tcpPacketClasses) {
      try {
        ...
        const shortName = fullName.split(".").pop();
        clazz.methods.forEach((method) => {
          if (["ToString", "GetHashCode", "Equals", "Finalize", ".cctor", ".ctor"].includes(method.name))
            return;
          try {
            clazz.method(method.name).implementation = function(...args) {
              const argStrs = args.map((a) => safeString(a));
...
function hookBinarySerialization(asm) {
  ...
  for (const fullName of binaryClasses) {
    try {
      ...
      const methods = isReader ? ["ReadInt32", "ReadUInt32", "ReadInt64", "ReadUInt64", "ReadString", "ReadBoolean", "ReadUInt16", "ReadByte"] : ["WriteInt32", "WriteUInt32", "WriteInt64", "WriteUInt64", "WriteString", "WriteBoolean", "WriteUInt16", "WriteByte"];
      for (const methodName of methods) {
        try {
          clazz.method(methodName).implementation = function(...args) {
...
function hookEncryption(asm) {
  ...
  try {
    ...
    try {
      rc4.method("Encrypt").implementation = function(data, key) {
...
\u{1F4C5} Authentication Timeline:`);
  ...
  console.log(`
\u{1F4CB} JSON Summary (copy for analysis):`);
  console.log("\u2500".repeat(66));
  const summary = {
    session: {
      duration: totalTime,
