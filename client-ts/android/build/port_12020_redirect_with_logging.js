...
class NativeTlsBypass {
  static enable(isDebugging = false) {
    ...
    if (isDebugging) {
      console.log(`[NativeTlsBypass] exports mbedtls_x509_crt_verify=${ptrInfo(mbedVerify)} mbedtls_x509_crt_verify_with_profile=${ptrInfo(mbedVerifyProfile)} mbedtls_x509_crt_verify_restartable=${ptrInfo(mbedVerifyRestartable)}`);
    }
...
console.log(`[SocketPatcher] exports getaddrinfo=${ptrInfo2(getaddrinfoPtr)} connect=${ptrInfo2(connectPtr)} __connect=${ptrInfo2(__connectPtr)} connect64=${ptrInfo2(connect64Ptr)} __connect64=${ptrInfo2(__connect64Ptr)} send=${ptrInfo2(sendPtr)} recv=${ptrInfo2(recvPtr)} sendto=${ptrInfo2(sendtoPtr)} recvfrom=${ptrInfo2(recvfromPtr)} write=${ptrInfo2(writePtr)} read=${ptrInfo2(readPtr)} __write_chk=${ptrInfo2(writeChkPtr)} __read_chk=${ptrInfo2(readChkPtr)} __sendto_chk=${ptrInfo2(sendtoChkPtr)} __recvfrom_chk=${ptrInfo2(recvfromChkPtr)} writev=${ptrInfo2(writevPtr)} readv=${ptrInfo2(readvPtr)} sendmsg=${ptrInfo2(sendmsgPtr)} recvmsg=${ptrInfo2(recvmsgPtr)} syscall=${ptrInfo2(syscallPtr)} socket=${ptrInfo2(socketPtr)} close=${ptrInfo2(closePtr)} shutdown=${ptrInfo2(shutdownPtr)} getsockopt=${ptrInfo2(getsockoptPtr)} poll=${ptrInfo2(pollPtr)} ntohs=${ptrInfo2(ntohsPtr)} inet_addr=${ptrInfo2(inet_addrPtr)} errno_fn=${ptrInfo2(errnoFnPtr)}`);
...
var socketMeta = new Map;
var pendingConnect = new Map;
var seenHostnames = new Set;
var hostnamesByIpv4 = new Map;
var hostnamesByIpv6 = new Map;
var ipsByHostname = new Map;
var recentWatchedHostByTid = new Map;
var observedEndpoints = new Map;
var captureConfig = {
  enabled: false,
  maxBytes: 2048,
  onlyPatched: true,
...
function maybeDecodePortStream(fd, dir, chunk) {
  ...
  if (state.mode === "unknown") {
    if (state.buffer.length >= 3 && state.buffer[0] === 22 && state.buffer[1] === 3) {
      ...
      const looksHttp = b0 === 71 && b1 === 69 && b2 === 84 && b3 === 32 || b0 === 80 && b1 === 79 && b2 === 83 && b3 === 84 || b0 === 72 && b1 === 84 && b2 === 84 && b3 === 80;
      if (looksHttp) {
...
// android/patchers/port_12020_redirect_with_logging.ts
...
console.log("╚═══════════════════════════════════════════════════════════════╝");
console.log("");
var DISCOVERY_DURATION_MS = 60000;
var authEvents = [];
var packetCaptures = [];
var dnsLookups = new Map;
var encryptionKeys = [];
      ...
      try {
        ...
        if (typeName === "Byte[]") {
          try {
            ...
            result[fieldName] = { type: "Byte[]", length: len };
          } catch (e) {
        ...
        if (value.class && depth < 2) {
          ...
          if (Object.keys(nested).length > 0) {
            ...
            result[fieldName] = { type: typeName };
          }
          return;
        }
        result[fieldName] = safeString(value);
      } catch (e) {
        result[field.name] = `<error: ${e.message || e}>`;
      }
    });
  } catch (e) {
...
function printFields(fields, indent = "│   ") {
  for (const [key, value] of Object.entries(fields)) {
    if (value && typeof value === "object" && value.fields) {
      ...
      const extra = value.length !== undefined ? `, len=${value.length}` : value.count !== undefined ? `, count=${value.count}` : "";
...
function hookTcpNetManager(asm) {
  ...
  try {
    ...
    try {
      tcpNetMgr.method("SendBuffer").implementation = function(buffer) {
    ...
    try {
      tcpNetMgr.method("HandleMsg").implementation = function(msgId, data) {
        ...
        logEvent({
          ...
          class: "TcpNetManager",
          method: "HandleMsg",
          data: { msgId: Number(msgId), length: len }
        });
