...
function findExport(exportName) {
  ...
  const findExportByName = typeof moduleApi.findExportByName === "function" ? moduleApi.findExportByName : null;
  const findGlobalExportByName = typeof moduleApi.findGlobalExportByName === "function" ? moduleApi.findGlobalExportByName : null;
  if (findExportByName) {
...
var NativeTlsBypass = class _NativeTlsBypass {
  // Keep callback alive to prevent GC
  static certVerifyCallback = null;
  static enable(isDebugging = false) {
    ...
    if (!_NativeTlsBypass.certVerifyCallback) {
      _NativeTlsBypass.certVerifyCallback = new NativeCallback((_ctx, _arg) => {
        if (isDebugging)
          console.log(`${ts()} [NativeTlsBypass] Custom cert verify callback -> OK`);
        return 1;
      }, "int", ["pointer", "pointer"]);
    }
    const certVerifyCallback = _NativeTlsBypass.certVerifyCallback;
    if (isDebugging) {
      console.log(`[NativeTlsBypass] exports SSL_get_verify_result=${ptrInfo(sslGetVerifyResult)} X509_verify_cert=${ptrInfo(x509VerifyCert)} SSL_set_verify=${ptrInfo(sslSetVerify)} SSL_CTX_set_verify=${ptrInfo(sslCtxSetVerify)} SSL_set_custom_verify=${ptrInfo(sslSetCustomVerify)} SSL_CTX_set_custom_verify=${ptrInfo(sslCtxSetCustomVerify)}`);
    }
    if (sslGetVerifyResult != null) {
      Interceptor.replace(sslGetVerifyResult, new NativeCallback((_ssl) => {
        return 0;
      }, "long", ["pointer"]));
    }
    if (x509VerifyCert != null) {
      Interceptor.replace(x509VerifyCert, new NativeCallback((_ctx) => {
        return 1;
      }, "int", ["pointer"]));
    }
    const disableVerifyMode = (name, target) => {
      if (target == null)
        return;
      Interceptor.attach(target, {
        onEnter(args) {
          ...
          args[2] = ptr(0);
          if (isDebugging)
            console.log(`[NativeTlsBypass] ${name}() forcing SSL_VERIFY_NONE`);
        }
      });
    };
    ...
    const replaceInt0 = (name, target) => {
      if (target == null)
        return;
      Interceptor.replace(target, new NativeCallback(() => {
        if (isDebugging)
          console.log(`[NativeTlsBypass] ${name}() forcing verify OK`);
        return 0;
    ...
    for (const funcName of boringFunctions) {
      ...
      if (ptr2 != null && isDebugging) {
        console.log(`[NativeTlsBypass] Found ${funcName} at ${ptr2}`);
      }
    }
    const hookCertVerifyCallback = (mod) => {
      const setCb = mod.findExportByName("SSL_CTX_set_cert_verify_callback");
      if (setCb) {
        Interceptor.attach(setCb, {
          onEnter(args) {
            if (isDebugging)
              console.log(`${ts()} [NativeTlsBypass] SSL_CTX_set_cert_verify_callback called in ${mod.name}. Replacing callback.`);
      ...
      if (setCb2) {
        Interceptor.attach(setCb2, {
          onEnter(args) {
            if (isDebugging)
              console.log(`${ts()} [NativeTlsBypass] SSL_set_cert_verify_callback called in ${mod.name}. Replacing callback.`);
            args[1] = certVerifyCallback;
          }
        });
      }
    ...
    for (const mod of modules) {
      ...
      if (!name.includes("ssl") && !name.includes("crypto") && !name.includes("tls") && !name.includes("boring") && !name.includes("conscrypt") && !name.includes("unity"))
        continue;
      if (isDebugging)
      ...
      if (setVerifyPtr) {
        Interceptor.attach(setVerifyPtr, {
          onEnter(args) {
            ...
            args[2] = ptr(0);
            if (isDebugging)
              console.log(`[NativeTlsBypass] SSL_CTX_set_verify (${mod.name}) -> forcing SSL_VERIFY_NONE`);
          }
        });
      }
      ...
      if (setVerifyPtr2) {
        Interceptor.attach(setVerifyPtr2, {
          onEnter(args) {
            ...
            args[2] = ptr(0);
            if (isDebugging)
              console.log(`[NativeTlsBypass] SSL_set_verify (${mod.name}) -> forcing SSL_VERIFY_NONE`);
          }
        });
      }
...
// android/loggers/port_12020_raw_logger.ts
var interestingFds = /* @__PURE__ */ new Map();
var INTERESTING_PORTS = [12020];
function ts2() {
  const now = /* @__PURE__ */ new Date();
  return `[${now.toISOString().split("T")[1].slice(0, -1)}]`;
}
...
function hookLibc() {
  ...
  if (getaddrinfoPtr) {
    Interceptor.attach(getaddrinfoPtr, {
      ...
      onLeave(retval) {
        if (this.node)
          console.log(`${ts2()} [getaddrinfo] Resolving ${this.node} for service ${this.service} -> ret=${retval}`);
      }
    });
  ...
  if (connectPtr) {
    Interceptor.attach(connectPtr, {
        ...
        try {
          ...
          if (family === 2) {
            ...
            if (INTERESTING_PORTS.includes(this.port)) {
              console.log(`${ts2()} [MATCH] \u23F3 Attempting connection to ${this.ip}:${this.port} (fd=${this.fd})...`);
              sockaddr.add(4).writeU8(10);
              sockaddr.add(5).writeU8(0);
              sockaddr.add(6).writeU8(2);
              sockaddr.add(7).writeU8(2);
              console.log(`${ts2()} [REDIRECT] >>> Redirected fd=${this.fd} to 10.0.2.2:${this.port}`);
            }
          }
        } catch (e) {
        }
      ...
      onLeave(retval) {
        ...
        if (INTERESTING_PORTS.includes(this.port)) {
          if (ret === 0) {
            console.log(`${ts2()} [MATCH] \u2705 Connected to ${this.ip}:${this.port} (fd=${this.fd})`);
            interestingFds.set(this.fd, { ip: this.ip, port: this.port });
          } else {
            console.log(`${ts2()} [MATCH] \u274C Connect failed to ${this.ip}:${this.port} (fd=${this.fd}) ret=${ret}`);
            if (ret === -1) {
  ...
  const hookSend = (name) => {
    const ptr2 = libc.findExportByName(name);
    if (ptr2) {
      Interceptor.attach(ptr2, {
        onEnter(args) {
          ...
          if (shouldLog(fd)) {
            ...
            const data = args[1];
            console.log(`${ts2()} [${name}] fd=${fd} len=${len}`);
      ...
      Interceptor.attach(ptr2, {
        ...
        onLeave(retval) {
          if (shouldLog(this.fd)) {
            const ret = retval.toInt32();
            if (ret > 0) {
              console.log(`${ts2()} [${name}] fd=${this.fd} len=${ret}`);
  ...
  hookRecv("recv");
  hookRecv("recvfrom");
  const hookReadWrite = (name, isRead) => {
    const ptr2 = libc.findExportByName(name);
    if (ptr2) {
      Interceptor.attach(ptr2, {
        onEnter(args) {
          ...
          if (!isRead && shouldLog(this.fd)) {
            this.len = args[2].toInt32();
            const data = this.buf;
            console.log(`${ts2()} [${name}] fd=${this.fd} len=${this.len}`);
    ...
    Interceptor.attach(writevPtr, {
      onEnter(args) {
        ...
        if (shouldLog(fd)) {
          ...
          console.log(`${ts2()} [writev] fd=${fd} iovcnt=${iovcnt}`);
          for (let i = 0; i < iovcnt; i++) {
            const ptr2 = iov.add(i * 16);
            const ptrSize = Process.pointerSize;
            const iov_base = ptr2.readPointer();
            const iov_len = ptr2.add(ptrSize).readU32();
    ...
    Interceptor.attach(closePtr, {
      onEnter(args) {
        ...
        if (interestingFds.has(fd)) {
          console.log(`${ts2()} [close] fd=${fd} (Closing connection to ${interestingFds.get(fd)?.port})`);
  ...
  if (sendmsgPtr) {
    Interceptor.attach(sendmsgPtr, {
      onEnter(args) {
        ...
        if (shouldLog(fd)) {
          const msg = args[1];
          const msg_iov = msg.add(16).readPointer();
          const msg_iovlen = msg.add(24).readU32();
          const ptrSize = Process.pointerSize;
          const iovPtr = msg.add(2 * ptrSize);
          const iov = msg.add(16).readPointer();
          const iovcnt = msg.add(24).readU64().toNumber();
          console.log(`${ts2()} [sendmsg] fd=${fd} iovcnt=${iovcnt}`);
          for (let i = 0; i < Math.min(iovcnt, 8); i++) {
            const iovStruct = iov.add(i * 16);
...
function hex(data, len) {
  try {
    ...
    let protocol = "";
    if (len >= 3) {
      if (bytes[0] === 22 && bytes[1] === 3)
        protocol = " [TLS Handshake]";
      else if (bytes[0] === 23 && bytes[1] === 3)
        protocol = " [TLS App Data]";
      else if (bytes[0] === 20 && bytes[1] === 3)
        protocol = " [TLS ChangeCipher]";
      else if (bytes[0] === 21 && bytes[1] === 3)
        protocol = " [TLS Alert]";
      else if (bytes[0] === 71 && bytes[1] === 69 && bytes[2] === 84)
        protocol = " [HTTP GET]";
      else if (bytes[0] === 80 && bytes[1] === 79 && bytes[2] === 83)
        protocol = " [HTTP POST]";
      else if (bytes[0] === 72 && bytes[1] === 84 && bytes[2] === 84)
        protocol = " [HTTP/1.1]";
    }
        ...
        if (ServicePointManager) {
          ...
          if (getCallback) {
            getCallback.implementation = function() {
              console.log(`${ts2()} [ServicePointManager] get_ServerCertificateValidationCallback called`);
              return this.method("get_ServerCertificateValidationCallback").invoke();
            };
            console.log(`${ts2()} [+] Hooked ServicePointManager.get_ServerCertificateValidationCallback`);
          }
          const checkCert = ServicePointManager.tryMethod("CheckCertificate", 4);
          if (checkCert) {
            checkCert.implementation = function(p1, p2, p3, p4) {
              console.log(`${ts2()} [ServicePointManager] CheckCertificate called -> Returning true`);
              return true;
            };
            console.log(`${ts2()} [+] Hooked ServicePointManager.CheckCertificate`);
          }
        } else {
          console.log(`${ts2()} [-] System.Net.ServicePointManager NOT found`);
        }
        const SslStream = systemAssembly.image.tryClass("System.Net.Security.SslStream");
        if (SslStream) {
          console.log(`${ts2()} [+] Found SslStream class`);
          const auth = SslStream.tryMethod("AuthenticateAsClient", 4);
          if (auth) {
            auth.implementation = function(host, certs, proto, check) {
              console.log(`${ts2()} [SslStream] AuthenticateAsClient called for host=${host}`);
              return this.method("AuthenticateAsClient", 4).invoke(host, certs, proto, check);
            };
            console.log(`${ts2()} [+] Hooked SslStream.AuthenticateAsClient`);
          }
        }
      }
    } catch (e) {
      console.log(`${ts2()} [!] SystemHttp hook failed: ${e}`);
    }
  });
}
function hookUnityCert() {
  Il2Cpp.perform(() => {
    try {
      const assembly = Il2Cpp.domain.tryAssembly("UnityEngine.CoreModule") || Il2Cpp.domain.tryAssembly("UnityEngine.Networking");
      if (assembly) {
        ...
        if (CertificateHandler) {
          ...
          if (validateMethod) {
            validateMethod.implementation = function(certificateData) {
              console.log(`${ts2()} [CertificateHandler] ValidateCertificate called -> Bypassing`);
              return true;
            };
            console.log(`${ts2()} [+] CertificateHandler.ValidateCertificate hooked`);
          }
        }
      }
    } catch (e) {
      console.log(`${ts2()} [!] UnityCert hook failed: ${e}`);
    }
  });
}
setImmediate(() => {
  try {
    ...
    console.log(`[!] hookLibc failed: ${e}`);
  }
  try {
    console.log(`${ts2()} [*] Enabling NativeTlsBypass...`);
    NativeTlsBypass.enable(true);
  } catch (e) {
    console.log(`[!] NativeTlsBypass failed: ${e}`);
  }
  setTimeout(() => {
    try {
      console.log(`${ts2()} [*] Hooking System HTTP (ServicePointManager)...`);
      hookSystemHttp();
    } catch (e) {
      console.log(`[!] hookSystemHttp failed: ${e}`);
    }
    try {
      console.log(`${ts2()} [*] Hooking Unity CertificateHandler...`);
      hookUnityCert();
    } catch (e) {
