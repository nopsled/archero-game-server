...
function msgTypeHex(msgType: number): string {
  return `0x${msgType.toString(16).padStart(4, "0")}`;
}

function msgTypeName(msgType: number): string {
  switch (msgType) {
    case MSG_TYPE_USER_LOGIN:
      return "CUserLoginPacket";
    case MSG_TYPE_USER_LOGIN_RESP:
...
function logPacket(
  ...
  console.log(
    `[TCP][${direction}] ${name} msgType=${msgTypeHex(msgType)} payloadBytes=${payloadLen}${extra ? ` ${extra}` : ""}`,
  );
}

function handlePacket(packet: Packet): OutgoingPacket | null {
  switch (packet.msgType) {
    case MSG_TYPE_USER_LOGIN: {
      try {
        ...
        logPacket(
          ...
          `transId=${loginReq.m_nTransID} platform=${JSON.stringify(loginReq.m_strPlatform)}`,
        );
        ...
        logPacket(
          "S->C",
          MSG_TYPE_USER_LOGIN_RESP,
          bytes.length,
          `transId=${loginResp.m_nTransID} userId=${loginResp.m_nUserRawId} level=${loginResp.m_nLevel} coins=${loginResp.m_nCoins} diamonds=${loginResp.m_nDiamonds}`,
        );

        const payload = Buffer.from(bytes);
        return {
          msgType: MSG_TYPE_USER_LOGIN_RESP,
          payload,
          buffer: createPacket(MSG_TYPE_USER_LOGIN_RESP, payload),
        };
      } catch (e) {
    ...
    default:
      logPacket("C->S", packet.msgType, packet.payload.length);
      logUnknownPacket(packet, "C->S");
      return null;
  }
}
 ...
 */
function logUnknownPacket(packet: Packet, direction: PacketDirection): void {
  const { msgType, payload } = packet;

  console.log(`\n${"=".repeat(80)}`);
  console.log(`[UNKNOWN PACKET][${direction}] Message Type: ${msgTypeHex(msgType)} (${msgType})`);
...
const tcpServer = net.createServer((socket) => {
  ...
  let expectedMsgType: number | null = null;
  let headerLogged = false;

  socket.on("data", (data) => {
    buffer = Buffer.concat([buffer, data]);

    // Best-effort peek of the first packet header so we can log what it is even if the
    // client disconnects before sending the full frame.
    if (expectedFrameBytes == null && buffer.length >= 4) {
      const length = buffer.readUInt32LE(0);
      expectedFrameBytes = 4 + length;
    }
    if (expectedMsgType == null && buffer.length >= 6) {
      expectedMsgType = buffer.readUInt16LE(4);
    }
    if (!headerLogged && expectedFrameBytes != null) {
      headerLogged = true;
      const name = expectedMsgType != null ? msgTypeName(expectedMsgType) : "UnknownPacket";
      const msgTypeStr = expectedMsgType != null ? msgTypeHex(expectedMsgType) : "unknown";
      console.log(
        `[TCP][C->S] Received ${data.length} bytes (buffer=${buffer.length}/${expectedFrameBytes}) next=${name} msgType=${msgTypeStr}`,
      );
    } else {
      console.log(`[TCP][C->S] Received ${data.length} bytes (buffer=${buffer.length} bytes)`);
    }
    ...
    while (result !== null) {
      ...
      if (response) {
        ...
        console.log(
          `[TCP][S->C] Sent ${msgTypeName(response.msgType)} msgType=${msgTypeHex(response.msgType)} frameBytes=${response.buffer.length} payloadBytes=${response.payload.length}`,
        );
      }
  ...
  socket.on("close", () => {
    if (buffer.length > 0) {
      const hintFrame =
        expectedFrameBytes != null ? ` expectedFrameBytes=${expectedFrameBytes}` : "";
      const hintType =
        expectedMsgType != null
          ? ` msgType=${msgTypeHex(expectedMsgType)} (${msgTypeName(expectedMsgType)})`
          : "";
      console.log(
        `[TCP][C->S] Connection closed with ${buffer.length} unparsed bytes${hintFrame}${hintType}`,
      );
    }
